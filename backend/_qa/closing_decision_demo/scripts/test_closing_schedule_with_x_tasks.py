#!/usr/bin/env python3
"""
Enhanced Closing Schedule Calculator Test Script with X Tasks
Tests the closing schedule calculator with real worker data AND X tasks for extended periods.
"""
...
def populate_workers_with_x_tasks(workers: List[EnhancedWorker], x_tasks: Dict[str, Dict[str, str]]):
    """Populate worker objects with their X tasks data."""
    for worker in workers:
        if worker.id in x_tasks:
            worker.x_tasks = x_tasks[worker.id].copy()
            print(f"  Populated {worker.name} with {len(worker.x_tasks)} X tasks")
        else:
            worker.x_tasks = {}
...
def analyze_closing_schedule(worker: EnhancedWorker, semester_weeks: List[date], 
    ...
    for close_date in required_dates:
        try:
            ...
            required_weeks.append(week_idx + 1)  # Convert to 0-based
        except ValueError:
            pass
    ...
    for close_date in optimal_dates:
        try:
            week_idx = semester_weeks.index(close_date)
            optimal_weeks.append(week_idx + 1)  # Convert to 0-based
        except ValueError:
            pass
...
def test_closing_schedule_with_x_tasks():
    """Test closing schedule calculator with real worker data AND X tasks for extended periods."""
    
    print("🔍 CLOSING SCHEDULE CALCULATOR TEST WITH X TASKS")
    print("=" * 80)
    ...
    # Populate workers with X tasks
    populate_workers_with_x_tasks(workers, x_tasks)
    ...
    test_periods = [
        (2025, 1, 26, "2025 Semester 1 (26 weeks) - WITH X TASKS"),
    ]
...
def test_specific_x_task_scenarios():
    ...
    workers_with_x_tasks = [w for w in workers if w.x_tasks]
...
if __name__ == "__main__":
    print("🚀 Starting Enhanced Closing Schedule Calculator Tests with X Tasks...")
    
    # Run comprehensive tests with X tasks
